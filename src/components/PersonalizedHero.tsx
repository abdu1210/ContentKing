...
const PersonalizedHero = ({ 
  ...
  const getBackgroundClass = () => {
    switch (heroData.background_style) {
      case 'gradient_secondary': return 'bg-gradient-secondary';
      case 'solid_background': return 'bg-background';
      ...
      <div className="container mx-auto px-4 py-20 relative z-10">
        {isCenteredLayout ? (
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-5xl lg:text-6xl font-bold leading-tight mb-6" data-cslp={cslp('hero_title')}>
              {heroData.hero_title?.split(' ').slice(0, -2).join(' ')}
              <span className={`block bg-gradient-to-r ${heroData.background_style === 'solid_background' ? 'from-primary to-accent' : 'from-white to-white/80'} bg-clip-text text-transparent`}>
                {heroData.hero_title?.split(' ').slice(-2).join(' ')}
              </span>
            </h1>
            {heroData.hero_subtitle && (
              <h2 className={`text-2xl mb-4 ${heroData.background_style === 'solid_background' ? 'text-muted-foreground' : 'text-white/90'}`} data-cslp={cslp('hero_subtitle')}>
                {heroData.hero_subtitle}
              </h2>
            )}
            <p className={`text-xl mb-8 leading-relaxed ${heroData.background_style === 'solid_background' ? 'text-muted-foreground' : 'text-white/90'}`} data-cslp={cslp('hero_description')}>
